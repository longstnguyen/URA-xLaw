...
Steps:
  ...
  4. Output → data/processed/unanswerable.jsonl

Usage:
  python -m ura_xlaw build-unanswerable --pick 100 --model gpt-4.1
  python -m ura_xlaw build-unanswerable --classify-only
"""
...
CLASSIFIER_QA_CORPUS = str(PATHS.retrieval_corpus)
FULL_CORPUS = str(PATHS.full_corpus)
...
UNANS_CHUNKS_SMALL = (
    f"{UNANS_OUT_DIR}/with_chunks_qa_only.jsonl"  # mapped vs qa_only (classifier)
)
UNANS_CHUNKS_FULL = (
    f"{UNANS_OUT_DIR}/with_chunks_full.jsonl"  # mapped vs full (for positives)
)
UNANS_FINAL = str(PATHS.processed / "unanswerable.jsonl")
...
def step1_pick_docs(n: int) -> int:
    ...
    used_in_2k = {str(record["id"]) for record in load_jsonl(SRC)}
...
def step2b_map_citations():
    ...
    classifier_corpus = (
        CLASSIFIER_QA_CORPUS if Path(CLASSIFIER_QA_CORPUS).exists() else QA_CORPUS
    )
    for tag, corpus, out in (
        ...
        cmd = [
            ...
            corpus,
            "--supp",
            str(PATHS.processed / "__nonexistent_supp__.parquet"),
            "--report",
            f"{UNANS_OUT_DIR}/coverage_{tag}.json",
        ]
        print(f"\n[map vs {tag}]  $ {' '.join(cmd)}\n")
...
def step3_classify_and_save():
    ...
    small = load_jsonl(UNANS_CHUNKS_SMALL)
    full = load_jsonl(UNANS_CHUNKS_FULL)
    ...
    pd.DataFrame(unans).to_parquet(
        UNANS_FINAL.replace(".jsonl", ".parquet"), index=False
    )
    ...
    if len(df_extra):
        df_extra["positives_json"] = df_extra["positives"].apply(
            lambda xs: json.dumps(xs, ensure_ascii=False)
        )
        df_extra.drop(columns=["positives"]).to_parquet(
            EXTRA_TRAIN.replace(".jsonl", ".parquet"), index=False
        )
...
def main():
    ...
    ap.add_argument(
        "--pick", type=int, default=100, help="Number of docs to pick & generate"
    )
    ...
    ap.add_argument(
        "--merge-train",
        action="store_true",
        help="After classify, merge answerable into train.jsonl",
    )
    ap.add_argument(
        "--expand-corpus",
        action="store_true",
...
def step5_expand_qa_corpus():
    ...
    print(
        f"  unique laws now: {expanded['law_sig'].nunique()} (was {qa['law_sig'].nunique()})"
    )
...
def step4_merge_into_train():
    ...
    existing = load_jsonl(train_jsonl)
